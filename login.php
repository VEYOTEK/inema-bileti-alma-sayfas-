...
if (isset($_POST['submit'])) {
    ...
    if ($user) {
        ...
        if (password_verify($parola, $user['parola'])) {
            // Oturum açma bilgilerini ayarla
            $_SESSION['kimlik'] = $user['kimlik'];
            $_SESSION['id'] = $user['id'];
            $_SESSION['ad'] = $user['ad'];
            $_SESSION['soyad'] = $user['soyad'];
            $_SESSION['yetki'] = $user['yetki'];
            $_SESSION['musteri'] = true;

            // Yetkiye göre yönlendirme
            if ($user['yetki'] == 1) { // Admin yetkisi
                header("Location: admin_filmler.php");
                exit();
            } if ($user['yetki'] == 0) { // Normal kullanıcı
                header("Location: musteri_paneli.php");
                exit();
            } else {
                echo "Geçersiz yetki düzeyi.";
            }
        } else {
            echo "Parola hatalı.";
        }
    } else {
        echo "Kimlik bulunamadı.";
    }
}
?>
<!DOCTYPE html>
...
<div class="login-card">
    ...
    <form method="POST" action="">
        ...
        <button type="submit" name="submit" class="shiny-cta">Giriş Yap</button>
        <div class="text-center mt-3">
            <a href="register.php"><span style="text-decoration:none;color:black;">Bir hesabın yokmu?</span>Kayıt Ol</a>
        </div>
    </form>
</div>
